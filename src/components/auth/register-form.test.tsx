...
describe("RegisterForm", () => {
	...
	it("keeps the confirmation out of the request body", async () => {
		const user = userEvent.setup();
		const fetchMock = stubFetch(201, { user: { username: "ada" } });
		render(<RegisterForm />);
	...
	it("submits once the mismatch is corrected", async () => {
		const user = userEvent.setup();
		const fetchMock = stubFetch(201, { user: { username: "ada" } });
		render(<RegisterForm />);
	...
	it("never echoes the submitted password back into the page text", async () => {
		...
		await user.type(screen.getByLabelText("Confirm password"), "hunter2secret");
		await user.click(screen.getByRole("button", { name: "Create account" }));
